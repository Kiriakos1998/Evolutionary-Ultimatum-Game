...
function model3Visualize(grid,generations,position,S1,S2)
...
p=rand(N+1,1);
q=rand(N+1,1);
%set all p to S1(1) and all q to S1(2)
p(1:1:N)=S1(1);
q(1:1:N)=S1(2);
%except from a small cluster
p(position)=S2(1);
q(position)=S2(2);
...
%visualize
    preshape=reshape(p(1:1:N)',grid,grid);
    imagesc(rot90(preshape),colorLimits);
    set(gca,'YDir','normal');
    colorbar
    figure(1)
    pause(0.5)
...
p(1:1:N,1)=p(competetorsT(parents),1);
q(1:1:N,1)=q(competetorsT(parents),1);
